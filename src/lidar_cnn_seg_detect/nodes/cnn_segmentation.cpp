...
bool CNNSegmentation::init()
{
  ...
  private_node_handle.param<double>("min_distance", min_distance_, 2.5);
  ROS_INFO("[%s] min_distance: %.2f", __APP_NAME__, min_distance_);

  private_node_handle.param<double>("max_distance", max_distance_, 60.);
  ROS_INFO("[%s] max_distance: %.2f", __APP_NAME__, max_distance_);

  private_node_handle.param<double>("clip_height", clip_height_, 10.0);
  ROS_INFO("[%s] clip_height: %.2f", __APP_NAME__, clip_height_);
...
void CNNSegmentation::pointsCallback(const sensor_msgs::PointCloud2 &msg)
{
  ...
  ROS_INFO_STREAM("process takes "<<elapsed<<" ms.");
}

void CNNSegmentation::remove_close_far_above_pt(const pcl::PointCloud<pcl::PointXYZI>::Ptr in,
...
#pragma omp for
    for (size_t i = 0; i < in->points.size(); i++)
    {
        ...
        //clip the points which is too far and too close
        if ((distance < min_distance_) || (distance > max_distance_))
        {
            indices.indices.push_back(i);
        }
        // clip the point which is tooo high
...
void CNNSegmentation::segDetected(const rockauto_msgs::DetectedObjectArray &objects, const pcl::PointCloud<pcl::PointXYZI>::Ptr &in_pc_ptr){

  pcl::ExtractIndices<pcl::PointXYZI> cliper;
  cliper.setInputCloud(in_pc_ptr);

  cliper.setIndices(boost::make_shared<pcl::PointIndices>(cluster2d_->indices_));
  cliper.setNegative(true); //ture to remove the indices

  pcl::PointCloud<pcl::PointXYZI>::Ptr out(new pcl::PointCloud<pcl::PointXYZI>);
  cliper.filter(*out);
  cluster2d_->indices_.indices.clear();

  pcl::PointCloud<pcl::PointXYZI>::Ptr final_pc(new pcl::PointCloud<pcl::PointXYZI>);
  remove_close_far_above_pt(out, final_pc);

  sensor_msgs::PointCloud2 output_cloud;
  pcl::toROSMsg(*final_pc, output_cloud);
  output_cloud.header = message_header_;
  mapping_points_pub_.publish(output_cloud);

  sensor_msgs::PointCloud2 original_cloud;
  pcl::toROSMsg(*in_pc_ptr, original_cloud);
  original_cloud.header = message_header_;
  original_points_pub_.publish(original_cloud);
}


void CNNSegmentation::pubColoredPoints(const rockauto_msgs::DetectedObjectArray &objects_array)
{
